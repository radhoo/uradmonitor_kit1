...
void make_dhcp_message_template(uint8_t *buf, const uint8_t transactionID) {
	...
	buf[UDP_DATA_P + 4] = 1;
	// the transaction ID is just some random number based on the last 2 MAC digits
	memset(buf + UDP_DATA_P + 5, transactionID, 3);
...
uint8_t send_dhcp_discover(uint8_t *buf, const uint8_t transactionID) {
	...
	buf[UDP_DATA_P + DHCP_OPTION_OFFSET] = 0x35; // 53 : DHCP MESSAGE TYPE
	buf[UDP_DATA_P + DHCP_OPTION_OFFSET + 1] = 1; //len
	buf[UDP_DATA_P + DHCP_OPTION_OFFSET + 2] = DHCP_DISCOVER_V;
	// option paramter request list:
	buf[UDP_DATA_P + DHCP_OPTION_OFFSET + 3] = 0x37; // 55 : PARAMETER REQUEST LIST
	...
	// add option hostname
	int len = strlen(g_hostname);
	if (len > 40) len = 40;
	if (len > 0) {
		buf[UDP_DATA_P + DHCP_OPTION_OFFSET + 7] = 12; // option HOSTNAME
		buf[UDP_DATA_P + DHCP_OPTION_OFFSET + 8] = len;
		memcpy(buf + UDP_DATA_P + DHCP_OPTION_OFFSET + 9, (uint8_t *) g_hostname, len);
		len += 2; // works even if len is 0
	}
	buf[UDP_DATA_P + DHCP_OPTION_OFFSET + 7 + len] = 0xff; // 255: end of options
	buf[UDP_DATA_P + DHCP_OPTION_OFFSET + 8 + len] = 0;

	// the length of the udp message part is now DHCP_OPTION_OFFSET+8 if no option 12 is set
	send_udp_transmit(buf, DHCP_OPTION_OFFSET + 8 + len);
...
uint8_t is_dhcp_msg_for_me(uint8_t *buf, uint16_t plen,
	...
	if (plen < 0x100)
		return (0);
	if (buf[UDP_SRC_PORT_L_P] != DHCP_SRV_SRC_PORT)
		return (0);
	if (buf[UDP_DATA_P] != 2)
		return (0); // message type DHCP boot reply =2
	if (buf[UDP_DATA_P + 5] != transactionID)
		return (0);
	if (buf[UDP_DATA_P + 6] != transactionID)
		return (0);
	return (1);
...
uint8_t send_dhcp_request(uint8_t *buf, const uint8_t transactionID) {
	...
	if (dhcp_opt_server_id[0] != 0) {
		...
		memcpy(buf + UDP_DATA_P + DHCP_OPTION_OFFSET + i + 2,
				dhcp_opt_server_id, 4);
		i = i + 6;
	}
	if (dhcp_yiaddr[0] != 0) {
	...
	if (len > 40) len = 40;
	if (len > 0) {
		buf[UDP_DATA_P + DHCP_OPTION_OFFSET + i + 4] = 12; // option HOSTNAME
		buf[UDP_DATA_P + DHCP_OPTION_OFFSET + i + 5] = len;
		memcpy(buf + UDP_DATA_P + DHCP_OPTION_OFFSET + i + 6, (uint8_t *) g_hostname, len);
		len += 2; // works even if len is 0
	}
	buf[UDP_DATA_P + DHCP_OPTION_OFFSET + i + 4 + len] = 0xff; // end of options
	buf[UDP_DATA_P + DHCP_OPTION_OFFSET + i + 5 + len] = 0;
	// the length of the udp message part is now DHCP_OPTION_OFFSET+i+5 if len is 0
	send_udp_transmit(buf, DHCP_OPTION_OFFSET + i + 5 + len);
	return (0);
}
...
uint8_t send_dhcp_renew_request(uint8_t *buf, const uint8_t transactionID,
		uint8_t *yiaddr) {
	make_dhcp_message_template(buf, transactionID);
...
uint8_t packetloop_dhcp_initial_ip_assignment(uint8_t *buf, uint16_t plen, uint8_t initial_tid, char *hostname) {
	...
	if (is_dhcp_msg_for_me(buf, plen, dhcp_tid)) {
		...
		// function call for. It could only happen if the board is power cycled
		// during operation.
		if (dhcp_is_renew_tid(buf, plen) == 1)
			return (0); // should have been initial tid, just return
...
void dhcp_get_my_ip(uint8_t *assigend_yiaddr, uint8_t *assigend_netmask,
		uint8_t *assigend_gw) {
	if (assigend_yiaddr)
		memcpy(assigend_yiaddr, dhcp_yiaddr, 4);
	if (assigend_netmask)
		memcpy(assigend_netmask, dhcp_opt_mask, 4);
	if (assigend_gw)
		memcpy(assigend_gw, dhcp_opt_defaultgw, 4);
}
...
uint16_t packetloop_dhcp_renewhandler(uint8_t *buf, uint16_t plen) {
	if (dhcp_6sec_cnt > 8) { // we let it run a bit faster than once every minute because it is better this expires too early than too late
		...
		if (dhcp_opt_leasetime_minutes < 0xffff
				&& dhcp_opt_leasetime_minutes > 1) {
			dhcp_opt_leasetime_minutes--;
		}
	}
	if (plen == 0 && dhcp_opt_leasetime_minutes < 3) {
		if (!enc28j60linkup())
			return (plen); // do nothing if link is down
		dhcp_tid++;
