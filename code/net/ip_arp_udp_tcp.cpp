...
static uint8_t (*client_tcp_result_callback)(uint8_t, uint8_t, uint16_t,
		uint16_t);
...
static char *client_additionalheaderline; // null pointer or pointer to a string
...
uint8_t client_tcp_req(
	...
	while (i < 4) {
		tcp_otherside_ip[i] = dstip[i];
		i++;
	}
	i = 0;
	while (i < 6) {
		tcp_dst_mac[i] = dstmac[i];
		i++;
	}
...
uint16_t www_client_internal_datafill_callback(uint8_t fd) {
	...
	if (fd == www_fd) {
		if (browsertype == 0) {
			...
			len = fill_tcp_data_p(bufptr, len, PSTR("Accept: */*\r\n"));
...
uint8_t www_client_internal_result_callback(uint8_t fd, uint8_t statuscode,
		uint16_t datapos, uint16_t len_of_data) {
	...
	if (statuscode == 0 && len_of_data > 12) {
		...
		if (client_browser_callback) {
			if (isblank(bufptr[datapos + 8]) && isdigit(bufptr[datapos + 9])
					&& isdigit(bufptr[datapos + 11])) { // e.g 200 OK, a status code has 3 digits from datapos+9 to datapos+11, copy over the web/http status code to web_statuscode:
				while (i < 2) {
...
//
void client_browse_url(const char *urlbuf_p, char *urlbuf_varpart,
		const char *hoststr, void (*callback)(uint16_t, uint16_t, uint16_t),
		uint8_t *dstip, uint8_t *dstmac) {
	if (!enc28j60linkup())
		return;
	...
	www_fd = client_tcp_req(&www_client_internal_result_callback,
			&www_client_internal_datafill_callback, 80, dstip, dstmac);
}
...
void client_http_post(const char *urlbuf_p, char *urlbuf_varpart,
		const char *hoststr, char *additionalheaderline, char *postval,
		void (*callback)(uint16_t, uint16_t, uint16_t), uint8_t *dstip,
		uint8_t *dstmac) {
	if (!enc28j60linkup())
		return;
	client_urlbuf_p = urlbuf_p;
...
uint16_t packetloop_arp_icmp_tcp(uint8_t *buf, uint16_t plen) {
	...
	if (plen == 0) {
		if (arpip_state == (WGW_ACCEPT_ARP_REPLY | WGW_INITIAL_ARP)
				&& arp_delaycnt == 0) {
		...
		if (tcp_client_state == 1 && enc28j60linkup()) { // send a syn
			...
			// from the server:
			tcp_client_syn(buf, ((tcp_fd << 5) | (0x1f & tcpclient_src_port_l)),
					tcp_client_port);
		}
#endif
		return (0);
...
#ifdef ARP_MAC_resolver_client
		if ((arpip_state & WGW_ACCEPT_ARP_REPLY)
				&& (buf[ETH_ARP_OPCODE_L_P] == ETH_ARP_OPCODE_REPLY_L_V)) {
			// is it an arp reply
			if (memcmp(&buf[ETH_ARP_SRC_IP_P], arpip, 4) != 0)
				return (0); // not an arp reply for the IP we were searching
			(*client_arp_result_callback)(arpip, arp_reference_number,
					buf + ETH_ARP_SRC_MAC_P);
	...
	if (buf[TCP_DST_PORT_H_P] == TCPCLIENT_SRC_PORT_H) {
		...
		if (buf[TCP_FLAGS_P] & TCP_FLAGS_RST_V) {
			if (client_tcp_result_callback) {
				...
				(*client_tcp_result_callback)(
						(buf[TCP_DST_PORT_L_P] >> 5) & 0x7, 3, 0, 0);
			}
			tcp_client_state = 6;
			return (0);
		}
		...
		if (tcp_client_state == 6) {
			...
			len++;
			if (buf[TCP_FLAGS_P] & TCP_FLAGS_ACK_V)
				len = 0; // if packet was an ack then do not step the ack number
			make_tcp_ack_from_any(buf, len, TCP_FLAGS_RST_V);
	...
	if (buf[TCP_DST_PORT_H_P] == wwwport_h
			&& buf[TCP_DST_PORT_L_P] == wwwport_l) {
		if (buf[TCP_FLAGS_P] & TCP_FLAGS_SYN_V) {
