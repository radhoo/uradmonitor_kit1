...
uint8_t BME280::uread8(uint8_t reg) {
	uint8_t buf;
	readmem(BME280_ADDR, reg, &buf, 1);
	return buf;
}

uint16_t BME280::uread16(uint8_t reg) {
	uint8_t buf[2];
	readmem(BME280_ADDR, reg, buf, 2);
...
bool BME280::begin(uint8_t a) {
	uint8_t id = 0;
	readmem(BME280_ADDR, BME280_REGISTER_CHIPID, &id, 1);
	if (id != 0x60) return false;

	getcalibration();
	writemem(BME280_ADDR, BME280_REGISTER_CONTROLHUMID, 0x03); // Set before CONTROL (DS 5.4.3)
	writemem(BME280_ADDR, BME280_REGISTER_CONTROL, 0x3F);
	return true;
}

void BME280::getcalibration() {
	uint8_t buffer[26];
	readmem(BME280_ADDR, BME280_REGISTER_T1, buffer, 26);
	...
	readmem(BME280_ADDR, BME280_REGISTER_H2, buffer, 7);
...
void BME280::readSensors(double *temperature, uint32_t *pressure, double *humidity) {
	unsigned char buffer[8];
	readmem(BME280_ADDR, BME280_REGISTER_PRESSUREDATA, buffer, 8);
